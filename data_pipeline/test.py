...
def match(expr, string):
    ...
    if not expr:
        ...
        print("Please enter the regex you would like to check in the format: 'regex'")    
        return [False, None, None]
...
def main():
    ...
    if matched:
        print(f'match regex: {regex}, string: {string}) = {string[match_pos:match_pos + match_length]}')
        print(f'match regex: {regex}, string: {string} = This test string CAN be generated by the given regex.')
    else:
        print(f'match_expr({regex}, {string}) = False')
